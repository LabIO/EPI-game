# falta extrair so um corpo
# falta determinar max range 
# limpar o buffer do fundo
# limpar o esqueleto
...
class BodyGameRuntime(object):
	...
	#self._client.connect(('192.168.43.217', 9000)) ## my pc with tethering from android
	#self._client.connect(('192.168.1.104', 5005)) ## rede X
	#self._client.connect(('192.168.2.120', 5005)) ## my pc with rede loveflat
	#self._client.connect(('127.0.0.1', 5005)) ## localhost
	#self._client.connect(('192.168.2.101', 5005)) ## ELisa pc with Loveflat       
	self._client.connect(('192.168.43.24', 5005)) ## rede X

	# set up OSC server
	#self._server_IP = '127.0.0.1' ## localhost
	#self._server_IP = '192.168.1.104'
	self._server_IP = '0.0.0.0' ## to receive from external
	self._server_PORT = 8080
	self._server_buffer_size = 1024
	self._my_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	self._my_socket.bind((self._server_IP,self._server_PORT))
	self._my_socket.setblocking(0)
	self._my_socket.settimeout(0.002)
	self._my_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, self._server_buffer_size)
        ...
        # here we will store skeleton data - initialization of the variable _bodies
        self._bodies = None
